                    ...
                    <div key={index} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow duration-300">
                        <div className="grid grid-cols-1 md:grid-cols-[0.5fr_2fr_1fr_1fr] gap-6 items-start">
                                ...
                                <div className="space-y-2 mb-4">
                                    {order.orderItems.map((item, idx) => (
                                        <div key={idx} className="flex items-center gap-2 text-sm">
                                            ...
                                            <span className="px-2 py-0.5 bg-gray-100 rounded text-xs text-gray-600">₹{item.price}</span>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="text-sm space-y-2">
                                ...
                                <p className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ${order.paymentStatus === 'paid' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                    <span className={`w-1.5 h-1.5 rounded-full ${order.paymentStatus === 'paid' ? 'bg-green-600' : 'bg-yellow-600'}`}></span>
